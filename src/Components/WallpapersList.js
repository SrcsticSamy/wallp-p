...
function WallpapersList() {
    ...
    const fetchWallpapers = async ({pageParam = null}) => {
        const res = await fetch(`${baseUrl}/${sort}.json?after=${pageParam}&limit=20${sort==="top"?"&t=all": null}`)
        return res.json()
    }

    const {isLoading, isFetchingNextPage, hasNextPage, isError, error, data, fetchNextPage} = useInfiniteQuery([queryName, sort], fetchWallpapers, 
    ...
    return (
        <Box>
            <Typography variant="h6" sx={{my:2}}>Sort By</Typography>

            <ButtonGroup aria-label="outlined primary button group" sx={{mb:8}}>
                <Button variant={sort==="hot"? "contained" : "outlined"} onClick={()=>setSort("hot")}>🔥 Hot</Button>
                <Button variant={sort==="top"? "contained" : "outlined"} onClick={()=>setSort("top")}>🚀 Top of All Time</Button>
                <Button variant={sort==="new"? "contained" : "outlined"} onClick={()=>setSort("new")}>✨ New</Button>
            </ButtonGroup>

            <Grid container justifyContent="space-evenly" maxWidth="false" sx={{mx:"auto", px:2}}>
